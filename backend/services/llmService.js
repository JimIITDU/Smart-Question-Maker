...
const llmService = {
  ...
  evaluateWrittenAnswer: async (questionText, studentAnswer, expectedAnswer, maxMarks) => {
    ...
    if (!model) {
      return llmService.mockEvaluateWrittenAnswer(questionText, studentAnswer, expectedAnswer, maxMarks);
    }
...
console.log('[LLM] Sending prompt to Gemini. Length:', prompt.length, 'chars');
      ...
      const marksObtained = Math.min(Math.max(parseFloat(evaluation.marks_obtained) || 0, 0), maxMarks);
      const confidenceScore = Math.min(Math.max(parseFloat(evaluation.confidence_score) || 0.7, 0), 1);
   ...
   */
  mockEvaluateWrittenAnswer: async (questionText, studentAnswer, expectedAnswer, maxMarks) => {
    ...
    if (score >= 0.9) {
      feedback = 'Excellent answer! You covered all the key points accurately and comprehensively. Your understanding of the topic is outstanding.';
    } else if (score >= 0.75) {
      feedback = 'Good answer! You covered most of the important points accurately. Minor improvements could include adding more detail about: ' + missingKeywords.slice(0, 3).join(', ') + '.';
    } else if (score >= 0.6) {
      feedback = 'Satisfactory answer. You demonstrated understanding of the core concepts but missed some important details. Consider including: ' + missingKeywords.slice(0, 4).join(', ') + '.';
    } else if (score >= 0.4) {
      feedback = 'Partial answer. You mentioned some relevant concepts but the response needs significant improvement. Key concepts to address: ' + missingKeywords.slice(0, 5).join(', ') + '.';
    } else if (score >= 0.2) {
      feedback = 'Limited answer. You touched on the topic but the response lacks depth and misses most key concepts. Please review: ' + missingKeywords.slice(0, 5).join(', ') + '.';
    } else {
      feedback = 'The answer does not adequately address the question. Please review the topic thoroughly and ensure you understand the fundamental concepts: ' + missingKeywords.slice(0, 5).join(', ') + '.';
    }
...
`;
      ...
      return questions.map(q => ({
        ...
        max_marks: parseInt(q.max_marks) || (question_type === 'descriptive' ? 5 : 2),
  ...
  mockGenerateQuestion: async (mode, params) => {
    ...
    const sampleQuestions = {
      physics: [
        ...
        {
          question_text: "Which of the following statements are true about acceleration due to gravity? (Select all that apply)",
          question_type: 'mcq',
          difficulty: 'medium',
          max_marks: 3,
      ...
      chemistry: [
        {
          question_text: "Select all noble gases from the following: (Select all that apply)",
          question_type: 'mcq',
          difficulty: 'easy',
          max_marks: 2,
      ...
      biology: [
        {
          question_text: "Which of the following are characteristics of living organisms? (Select all that apply)",
          question_type: 'mcq',
          difficulty: 'easy',
          max_marks: 2,
    ...
    switch (mode) {
      case 'random':
        questions = Array.from({ length: count }, (_, i) => {
          ...
          const q = {
            ...
            question_type,
            difficulty,
            max_marks: question_type === 'descriptive' ? 5 : (question_type === 'mcq' ? 2 : 1),
            source: 'ai_random'
          };
          if (isMCQ) {
      ...
      case 'guided':
        questions = Array.from({ length: count }, (_, i) => {
          const isMCQ = question_type === 'mcq';
          const hasMultipleHint = hints.toLowerCase().includes('multiple') || hints.toLowerCase().includes('all that apply');
          const q = {
      ...
      case 'zero-shot':
        ...
        const subjectQuestions = sampleQuestions[subjectKey] || sampleQuestions.physics;
