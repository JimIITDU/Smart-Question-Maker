...
function shuffleQuestionOptions(question, seed) {
  ...
  const originalCorrect = question.correct_option ? question.correct_option.trim().toUpperCase() : '';
  const correctTexts = originalCorrect.split(',').map(s => s.trim()).filter(Boolean);
...
async function generateExamPDF(exam, center, questions, setLabel) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(18)
        .font('Helvetica-Bold')
        .text(center?.center_name || 'Coaching Center', 50, 50, { align: 'center', width: pageWidth });

      // Decorative line
      doc.moveTo(50, 75).lineTo(doc.page.width - 50, 75).stroke('#2563eb');
      doc.moveDown(0.5);

      // Exam Title and Set Label
      doc.fontSize(16)
        .font('Helvetica-Bold')
        .text(`${exam.title || 'Untitled Exam'}`, { align: 'center', width: pageWidth });
      ...
      doc.text(`Duration: ${exam.duration_minutes || 60} minutes`, 60 + colWidth, infoY + 8);
      doc.text(`Total Marks: ${calculateTotalMarks(questions)}`, 60 + colWidth * 2, infoY + 8);

      doc.text(`Batch: ${exam.batch_name || 'N/A'}`, 60, infoY + 32);
      doc.text(`Date: ${exam.start_time ? new Date(exam.start_time).toLocaleDateString() : '__________'}`, 60 + colWidth, infoY + 32);
      doc.text(`Access Code: ${exam.access_code || 'N/A'}`, 60 + colWidth * 2, infoY + 32);
      ...
      fields.forEach((field, i) => {
        ...
        doc.moveTo(x + doc.widthOfString(`${field.label}: `), y + 12)
      ...
      doc.text('4. Do not write anything on this question paper except your name and roll number.', { indent: 10 });
      doc.text('5. Use of calculators or mobile phones is strictly prohibited.', { indent: 10 });
      ...
      // === QUESTIONS SECTION ===
      doc.fontSize(13).font('Helvetica-Bold').text('QUESTIONS', { align: 'center' });
      doc.moveDown(1);
      ...
      questions.forEach((q, idx) => {
        ...
        if (doc.y > doc.page.height - 150) {
          doc.addPage();
          doc.fontSize(10).fillColor('#666666').text(`Set ${setLabel} - ${exam.title || 'Exam'}`, 50, 30);
          doc.moveTo(50, 45).lineTo(doc.page.width - 50, 45).stroke('#eeeeee');
          doc.y = 55;
          doc.fillColor('#000000');
        }
        ...
        doc.text(q.question_text, 80, questionY, { width: questionTextWidth, align: 'left' });
        ...
        doc.text(`[${q.max_marks || 1} mark${q.max_marks > 1 ? 's' : ''}]`, 80, metaY);
        ...
        if (q.question_type === 'mcq') {
          ...
          doc.rect(80, answerBoxY, doc.page.width - 160, boxHeight).stroke('#dddddd');
...
async function generateAnswerKeyPDF(exam, center, questions, setLabel) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.moveTo(50, 75).lineTo(doc.page.width - 50, 75).stroke('#16a34a');
      doc.moveDown(0.5);

      doc.fontSize(16).font('Helvetica-Bold')
        .text(`${exam.title || 'Untitled Exam'}`, { align: 'center', width: pageWidth });

      doc.fontSize(14).font('Helvetica').fillColor('#dc2626')
        .text(`ANSWER KEY - SET ${setLabel}`, { align: 'center', width: pageWidth });

      doc.fillColor('#000000');
      doc.moveDown(1);

      doc.fontSize(10).font('Helvetica')
        .text(`Subject: ${exam.subject_name || 'N/A'}  |  Duration: ${exam.duration_minutes || 60} mins  |  Total Marks: ${calculateTotalMarks(questions)}`, { align: 'center' });
      ...
      doc.moveTo(50, tableY + 18).lineTo(doc.page.width - 50, tableY + 18).stroke('#333333');
      doc.moveDown(1.5);

      questions.forEach((q, idx) => {
        if (doc.y > doc.page.height - 100) {
          doc.addPage();
          doc.fontSize(10).fillColor('#16a34a').text(`Answer Key - Set ${setLabel} - ${exam.title || 'Exam'}`, 50, 30);
          doc.moveTo(50, 45).lineTo(doc.page.width - 50, 45).stroke('#eeeeee');
          doc.y = 55;
          doc.fillColor('#000000');
        }
        ...
        const textHeight = doc.heightOfString(modelText, { width: doc.page.width - 480 });
...
async function generateExamSets(exam, center, questions, numSets = 2) {
  ...
  for (let i = 0; i < setsCount; i++) {
    ...
    results.push({
      ...
      examPDF,
      answerKeyPDF,
      examFilename: `${exam.title || 'Exam'}_Set_${setLabel}.pdf`.replace(/\s+/g, '_'),
      answerKeyFilename: `${exam.title || 'Exam'}_Answer_Key_Set_${setLabel}.pdf`.replace(/\s+/g, '_'),
    });
  }
