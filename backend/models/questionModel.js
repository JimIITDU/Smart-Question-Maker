...
const questionModel = {
  ...
  createQuestion: async (data) => {
    const {
      ...
      status, negative_marks, explanation, chapter_id, options, correct_answers,
    } = data;

    // Parse options if passed as JSON string
    let parsedOptions = options;
    if (typeof options === 'string') {
      try { parsedOptions = JSON.parse(options); } catch (e) { parsedOptions = null; }
    }

    // Parse correct_answers if passed as JSON string
    let parsedCorrectAnswers = correct_answers;
    if (typeof correct_answers === 'string') {
      try { parsedCorrectAnswers = JSON.parse(correct_answers); } catch (e) { parsedCorrectAnswers = null; }
    }
    ...
    const baseColumns = [
      ...
      'question_text', 'question_type', 'difficulty', 'expected_answer', 'max_marks', 'option_text_a', 'option_text_b',
      'option_text_c', 'option_text_d', 'correct_option', 'is_multiple_correct', 'created_by', 'source', 'status'
    ];
    const baseValues = [
      coaching_center_id, subject_id, course_id, class_name, subject_name, paper, chapter, chapter_name, topic,
      question_text, question_type, difficulty, expected_answer, max_marks, optA, optB,
      optC, optD, corrOpt, is_multiple_correct || false, created_by, source, status || 'active'
    ];

    // Optional columns that may not exist in older databases
    const optionalColumns = ['negative_marks', 'explanation', 'chapter_id', 'options', 'correct_answers'];
    const optionalValues = [negative_marks || 0, explanation || null, chapter_id || null, parsedOptions || null, parsedCorrectAnswers || null];
  ...
  updateQuestion: async (id, data, teacher_id, coaching_center_id) => {
    ...
    const {
      ...
      subject_id, course_id, status, negative_marks, explanation, chapter_id,
    ...
    const baseUpdates = [
      'question_text=$1', 'question_type=$2', 'difficulty=$3',
      ...
      'subject_id=$18', 'course_id=$19', 'status=$20'
    ];
    const baseValues = [
      question_text, question_type, difficulty, expected_answer,
      max_marks, optA, optB,
      optC, optD, corrOpt, is_multiple_correct || false,
      class_name, subject_name, paper, chapter, chapter_name, topic,
      subject_id, course_id, status
    ];
  ...
  getRandomQuestions: async (coaching_center_id, subject_id, difficulty, limit, teacher_id) => {
...
saveAIGeneratedQuestions: async (questions, teacher_id, coaching_center_id) => {
    const ids = [];
    for (const q of questions) {
   ...
   */
  saveAIGeneratedQuestionsWithMeta: async (questions, teacher_id, coaching_center_id, metadata) => {
    const ids = [];
    for (const q of questions) {
      const id = await questionModel.createQuestion({
        ...q,
   ...
   */
  bulkUpdateStatusWithDifficulty: async (updates, teacher_id, coaching_center_id) => {
    const results = [];
    for (const update of updates) {
      ...
      const result = await db.query(
        `UPDATE question_bank 
         ...
         RETURNING question_id`,
        [update.status, update.difficulty || null, update.id, teacher_id, coaching_center_id]
      );
      if (result.rows.length > 0) {
        results.push(result.rows[0].question_id);
      }
