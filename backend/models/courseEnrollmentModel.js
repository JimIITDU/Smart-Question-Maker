...
const courseEnrollmentModel = {
  ...
  browseCourses: async (filters = {}) => {
    ...
    if (search) {
      conditions.push(
        `(c.course_title ILIKE $${paramIndex} OR c.course_description ILIKE $${paramIndex})`,
      );
      params.push(`%${search}%`);
      paramIndex++;
    }
  ...
  enrollStudent: async (student_id, course_id) => {
    ...
    if (course.fee === 0 || course.fee === null || course.fee === undefined) {
      ...
      const expiresAt =
        course.end_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
  ...
  getCourseDetail: async (course_id, student_id = null) => {
    ...
    const scheduled = examsResult.rows.filter(
      (e) => e.exam_type === "scheduled",
    );
    const liveQuiz = examsResult.rows.filter(
      (e) => e.exam_type === "live_quiz",
    );
  ...
  updateCourse: async (course_id, data, coaching_center_id) => {
    ...
    const result = await db.query(`SELECT * FROM course WHERE course_id = $1`, [
      course_id,
    ]);
    return result.rows[0];
  },
