...
const courseEnrollmentController = {
  ...
  getCourseExams: async (req, res) => {
    try {
      ...
      const exams = await courseEnrollmentModel.getCourseExams(
        parseInt(course_id),
      );
  ...
  getAdminCourses: async (req, res) => {
    try {
      const coaching_center_id =
        req.user.coaching_center_id || req.tenant?.coaching_center_id;
      ...
      const courses =
        await courseEnrollmentModel.getAdminCourses(coaching_center_id);
  ...
  getPaymentDetails: async (req, res) => {
    try {
      ...
      const enrollment =
        await courseEnrollmentModel.getEnrollmentById(enrollmentId);
      if (!enrollment) {
        return res.status(404).json({
          success: false,
  ...
  getCourseStudentsLegacy: async (req, res) => {
    try {
      ...
      const isAdmin =
        center && center.coaching_center_id === course.coaching_center_id;
