...
async function migrate() {
  ...
  try {
    ...
    if (!(await columnExists(client, 'question_bank', 'status'))) {
      await client.query(`ALTER TABLE question_bank ADD COLUMN status VARCHAR(20) DEFAULT 'active'`);
    ...
    if (!(await columnExists(client, 'question_bank', 'negative_marks'))) {
      await client.query(`ALTER TABLE question_bank ADD COLUMN negative_marks DECIMAL(5,2) DEFAULT 0`);
    ...
    if (!(await columnExists(client, 'question_bank', 'explanation'))) {
      await client.query(`ALTER TABLE question_bank ADD COLUMN explanation TEXT`);
    ...
    if (!(await columnExists(client, 'question_bank', 'chapter_id'))) {
      await client.query(`ALTER TABLE question_bank ADD COLUMN chapter_id INTEGER`);
    ...
    if (!(await columnExists(client, 'question_bank', 'correct_answers'))) {
      await client.query(`ALTER TABLE question_bank ADD COLUMN correct_answers JSONB`);
