...
const SubscriptionManagement = () => {
  ...
  const handleUpgrade = async (planId) => {
    if (!window.confirm("Are you sure you want to upgrade your subscription?"))
      return;
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-400">
            Choose the right plan for your coaching center
          </p>
        </div>
        ...
        {currentSubscription?.status !== "active" && (
          <div className="mb-6 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <p className="text-amber-400 text-sm">
              Your center must be approved before you can upgrade your
              subscription.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, i) => {
            ...
            return (
              <div
                key={plan.plan_id || i}
                className={`bg-white/5 border rounded-2xl p-8 transition-all ${isCurrentPlan ? "border-teal-500/30" : "border-white/10"} ${i === 2 ? "relative overflow-hidden" : ""}`}
              >
                {i === 2 && (
