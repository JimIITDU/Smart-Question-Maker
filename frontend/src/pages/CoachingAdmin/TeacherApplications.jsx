...
import { getCenterApplications, approveApplication, rejectApplication, getAvailableTeachers } from '../../services/api';
        ...
        {activeTab === 'applications' && (
          <div>
            {pendingApps.length === 0 ? (
              ...
              pendingApps.map(app => (
                <div key={app.application_id} className="bg-white shadow rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-semibold text-lg">{app.teacher_name}</h3>
                      <p className="text-gray-600">{app.teacher_email}</p>
                      <div className="mt-2">
                        <p><strong>Specialization:</strong> {app.subjects_specialization}</p>
                        <p><strong>Experience:</strong> {app.experience_years} years</p>
                        <p><strong>Bio:</strong> {app.bio}</p>
                        {app.expected_salary && <p><strong>Expected Salary:</strong> {app.expected_salary}</p>}
          ...
          <div>
            {processedApps.length === 0 ? (
              ...
              processedApps.map(app => (
                <div key={app.application_id} className={`bg-white shadow rounded-lg p-4 mb-4 ${
                  app.status === 'approved' ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500'
                }`}>
                  <h3 className="font-semibold text-lg">{app.teacher_name}</h3>
                  <p className="text-gray-600">{app.teacher_email}</p>
                  <p><strong>Status:</strong> {app.status}</p>
                  {app.reviewed_at && (
                    <p className="text-sm text-gray-500">Reviewed: {new Date(app.reviewed_at).toLocaleDateString()}</p>
                  )}
                </div>
              ))
            )}
          ...
          <div>
            {availableTeachers.length === 0 ? (
              ...
              availableTeachers.map(teacher => (
                <div key={teacher.user_id} className="bg-white shadow rounded-lg p-4 mb-4">
                  <h3 className="font-semibold text-lg">{teacher.name}</h3>
                  <p className="text-gray-600">{teacher.email}</p>
                  <p><strong>Specialization:</strong> {teacher.subject_specialization || 'N/A'}</p>
                  <p><strong>Experience:</strong> {teacher.experience || 0} years</p>
                </div>
              ))
            )}
          </div>
