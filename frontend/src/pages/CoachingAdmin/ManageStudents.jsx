...
const ManageStudents = () => {
  ...
  const fetchStudents = async (batchId) => {
    try {
      const res = await fetch(
        `https://smart-question-maker-backend.onrender.com/api/academic/batches/${batchId}/students`,
        {
  ...
  const handleEnroll = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://smart-question-maker-backend.onrender.com/api/academic/enroll",
        {
          method: "POST",
          headers: {
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">
            Manage <span className="text-emerald-400">Students</span>
          </h1>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEnrollData({ ...enrollData, batch_id: e.target.value })
                  }
                  required
                  className="w-full bg-[#0B1120] border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500"
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEnrollData({ ...enrollData, user_id: e.target.value })
                  }
                  required
                  placeholder="e.g. 10"
                  className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500"
                />
              </div>
            </div>
            <button
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="w-full md:w-64 bg-[#0B1120] border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl">
            ...
            <p className="text-gray-500">
              Select a batch to view enrolled students
            </p>
          </div>
        ) : students.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {students.map((student) => (
                <div
                  key={student.user_id}
                  className="flex items-center justify-between p-5 hover:bg-white/5 transition-all"
                >
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-400">
                    {student.roll_number && (
                      <span className="bg-white/5 px-3 py-1 rounded-full font-mono">
                        {student.roll_number}
                      </span>
                    )}
