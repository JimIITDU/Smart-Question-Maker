  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        ...
        const teachersRes = await getAdminUsers({ role: 3, search: "", page: 1 });
        const centerTeachers = (teachersRes.data.data || []).filter(
          (u) => u.coaching_center_id === parseInt(id) && u.is_active,
        );
        setStats((prev) => ({ ...prev, teachersCount: centerTeachers.length }));

        // Students: role 5
        const studentsRes = await getAdminUsers({ role: 5, search: "", page: 1 });
        const centerStudents = (studentsRes.data.data || []).filter(
          (u) => u.coaching_center_id === parseInt(id) && u.is_active,
        );
        setStats((prev) => ({ ...prev, studentsCount: centerStudents.length }));

        // TODO: backend endpoint for exams count
        setStats((prev) => ({ ...prev, examsCount: 0 }));
      } catch (error) {
                ...
                <div>
                  ...
                  {center.plan_name && (
                    <div className="mt-2 p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                      ...
                      {center.price && (
                        <span className="text-sm ml-2 text-emerald-300">
                          ({center.price ? `৳${center.price}/mo` : "Free"})
                        </span>
                      )}
                    </div>
                  )}
