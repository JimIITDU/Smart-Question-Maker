...
const ManageSubscriptionPlans = () => {
    ...
    try {
      const data = {
        ...
        max_courses: formData.max_courses
          ? parseInt(formData.max_courses)
          : null,
        max_exams: formData.max_exams ? parseInt(formData.max_exams) : null,
        ai_questions_limit: formData.ai_questions_limit
  ...
  const handleToggle = async (plan) => {
    try {
      await toggleSubscriptionPlanStatus(plan.plan_id, {
      ...
      toast.error(
        err.response?.data?.message || "Failed to toggle plan status",
      );
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20">
        ...
        {showForm && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
                  AI Questions Limit/month
                </label>
                <input
                  type="number"
                  name="ai_questions_limit"
                  value={formData.ai_questions_limit}
                  onChange={handleChange}
                  placeholder="0 for unlimited, leave empty for no AI"
                  className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500"
                />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={2}
                placeholder="Unlimited students, Custom domain, Priority support"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <div
              key={plan.plan_id}
              className={`bg-white/5 border rounded-2xl p-8 transition-all hover:border-white/20 ${!plan.is_active ? "opacity-60" : ""}`}
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`transition-colors ${plan.is_active ? "text-emerald-500 hover:text-emerald-400" : "text-gray-500 hover:text-gray-400"}`}
                  >
                    {plan.is_active ? (
              ...
              <ul className="space-y-3 mb-6">
                ...
                {plan.ai_questions_limit !== undefined && (
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <FiCheck className="text-emerald-400 flex-shrink-0" />
                    {plan.ai_questions_limit === 0
                      ? "No AI questions"
                      : plan.ai_questions_limit
                        ? `AI: ${plan.ai_questions_limit}/mo`
              ...
              <div
                className={`text-center py-2 rounded-xl text-xs font-bold ${plan.is_active ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"}`}
              >
