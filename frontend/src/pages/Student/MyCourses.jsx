...
const MyCourses = () => {
  ...
  const getDaysBadge = (days) => {
    if (days === null) {
      return <span className="px-2 py-1 bg-gray-700 text-gray-400 text-xs rounded-full">No expiry</span>;
    }
    if (days > 30) {
      return <span className="px-2 py-1 bg-emerald-900 text-emerald-400 text-xs rounded-full">{days}d left</span>;
    }
    if (days >= 7) {
      return <span className="px-2 py-1 bg-amber-900 text-amber-400 text-xs rounded-full">{days}d left</span>;
    }
    if (days >= 0) {
      return <span className="px-2 py-1 bg-red-900 text-red-400 text-xs rounded-full">{days}d left</span>;
    }
    return <span className="px-2 py-1 bg-gray-700 text-gray-400 text-xs rounded-full">Expired</span>;
  };

  const getStatusBadge = (status, daysRemaining) => {
    if (status !== 'active' || (daysRemaining !== null && daysRemaining < 0)) {
      return <span className="px-2 py-1 bg-red-900 text-red-400 text-xs rounded-full">Expired</span>;
    }
    return <span className="px-2 py-1 bg-emerald-900 text-emerald-400 text-xs rounded-full">Active</span>;
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {courses.length === 0 ? (
          <div className="bg-gray-800 rounded-xl p-12 text-center">
            ...
            <p className="text-gray-400 mb-6">You haven't enrolled in any courses. Start learning today!</p>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map(course => {
              ...
              return (
                <div key={course.enrollment_id || course.course_id} className="bg-gray-800 rounded-xl overflow-hidden hover:ring-2 hover:ring-purple-500 transition-all">
                  {/* Card Header */}
                  <div className="p-5">
                  ...
                  <div className="px-5 py-4 bg-gray-900/50">
                    {(course.status === 'active' && (daysRemaining === null || daysRemaining >= 0)) ? (
                      <button
