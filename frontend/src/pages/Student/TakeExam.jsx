...
const TakeExam = () => {
  ...
  const handleSubmit = async (isAuto = false) => {
    ...
    if (!isAuto && questions.length > answeredCount && !submitting) {
      if (
        !window.confirm(
          `You have only answered ${answeredCount} out of ${questions.length} questions. Are you sure you want to submit?`,
        )
      ) {
        return;
      }
  ...
  const isQuestionAnswered = (q) => {
    ...
    if (q.question_type === "descriptive") {
      return (
        ...
        answer.descriptive_answer.trim().length > 0
      );
    }
    return false;
  };
      ...
      <header className="sticky top-0 z-50 bg-[#0B0C15]/90 backdrop-blur-xl border-b border-white/5 shadow-lg">
        <div className="max-w-4xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div
              className={`px-4 py-2 rounded-xl border font-mono text-lg font-bold tracking-wider flex items-center gap-2 ${
                timeLeft < 60
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <span
                      className={`flex items-center justify-center w-8 h-8 rounded-lg font-bold text-sm border ${
                        isAnswered
                  ...
                  <div className="space-y-3">
                    ...
                    {["A", "B", "C", "D"].map((opt) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors shrink-0 pointer-events-none ${
                              isSelected
                          ...
                          <div className="flex-1 pointer-events-none">
                            <span className="text-xs font-bold text-gray-500 mb-0.5 block">
                              Option {opt}
                            </span>
                            <span
                              className={`text-base font-medium ${isSelected ? "text-white" : "text-gray-300"}`}
                            >
                              {optionText}
                            </span>
                          </div>
                    ...
                    {currentAnswer?.selected_options?.length > 0 && (
                      <p className="text-xs text-indigo-400 mt-1">
                        Selected:{" "}
                        {currentAnswer.selected_options.sort().join(", ")}
                      </p>
                    )}
                  </div>
                )}
                    ...
                    {["True", "False"].map((opt) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isSelected && (
                            <CheckCircle className="w-5 h-5 text-indigo-400 pointer-events-none" />
                          )}
                        </label>
                      );
                    })}
