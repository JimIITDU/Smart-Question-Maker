...
import { browseCourses, enrollInCourse, checkEnrollment, getMyCourses } from '../../services/api';
...
const BrowseCourses = () => {
    ...
    try {
      ...
      if (user) {
        try {
          ...
          const enrolledIds = (myCoursesRes.data.data || []).map(c => c.course_id);
  ...
  const handleEnroll = async (course) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || error.response?.data?.error || 'Failed to enroll');
    }
  };
  ...
  const getEnrollmentTypeBadge = (type) => {
    if (type === 'private') {
      return <span className="px-2 py-1 bg-red-900 text-red-200 text-xs rounded-full">Private</span>;
    }
    return <span className="px-2 py-1 bg-blue-900 text-blue-200 text-xs rounded-full">Open</span>;
  };
  ...
  return (
    <div className="min-h-screen bg-[#030712] p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Browse Courses</h1>
          <p className="text-gray-400">Discover courses from coaching centers</p>
        </div>
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map(course => (
              <div key={course.course_id} className="bg-gray-800 rounded-xl overflow-hidden hover:ring-2 hover:ring-purple-500 transition-all">
                {/* Card Header */}
                <div className="p-5">
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    <div className="flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354l-8 4.5 8 4.5 8-4.5-8-4.5zM4.5 10.854L12 15.354l7.5-4.5-7.5-4.5z" />
                      </svg>
