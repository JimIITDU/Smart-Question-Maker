        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0B0C15]/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[#13151f] border border-indigo-500/30 rounded-3xl p-8 max-w-md w-full shadow-[0_0_50px_-12px_rgba(79,70,229,0.5)] text-center">
            ...
            <p className="text-gray-400 mb-6">
              Share this access code with your students to let them join the
              live quiz.
            </p>

            <div className="bg-[#0B0C15] border border-white/10 rounded-2xl p-6 mb-6 group relative">
              <span className="text-[10px] uppercase tracking-[0.2em] text-gray-500 block mb-2">
                Access Code
              </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  >
                    <option value="regular" className="bg-[#0B0C15]">
                      Regular
                    </option>
                    <option value="live_quiz" className="bg-[#0B0C15]">
                      Live Quiz
                    </option>
                  </select>
                </div>
              ...
              {formData.exam_type === "live_quiz" && (
                <div className="mt-4 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-center">
                  <p className="text-sm text-indigo-400">
                    Live Quiz mode selected. An access code will be generated
                    after creation.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <div className="bg-[#0B0C15] border border-white/10 rounded-xl p-4 max-h-60 overflow-y-auto space-y-2 custom-scrollbar">
                  {questions.length > 0 ? (
                    questions.map((q) => {
                      const isSelected = formData.question_ids.includes(
                        q.question_id,
                      );
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-center gap-3 overflow-hidden">
                            <div
                              className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-indigo-500 border-indigo-500" : "border-gray-600"}`}
                            >
                              {isSelected && (
                                <svg
                          ...
                          <span
                            className={`ml-4 px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                              q.difficulty === "easy"
            ...
            {exams.map((exam) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`absolute top-0 right-0 mt-4 mr-4 w-2.5 h-2.5 rounded-full shadow-[0_0_10px_currentColor] ${status.color}`}
                  ></div>

                  {/* Header */}
                  <div className="flex items-start justify-between mb-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${status.border} ${status.text}`}
                    >
                      {status.label}
                    </span>
