  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.exam_type === "live_quiz" && formData.start_time) {
        ...
        const end = new Date(
          start.getTime() + (formData.hours_open || 24) * 60 * 60 * 1000,
        );
      ...
      <main className="max-w-4xl mx-auto px-6 pt-28 pb-20">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 [color-scheme:dark]"
                />
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      min={1}
                      max={168}
                      className="w-full bg-white/5 border border-white/10 text-white rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:border-purple-500"
                      placeholder="24"
                    />
                  </div>
                  <p className="text-xs text-gray-500 mt-1">
                    Quiz available for this many hours after start
                  </p>
                </div>
              ) : (
                <div>
            ...
            {/* Description helper */}
            <div
              className={`p-4 rounded-xl border ${isLiveQuiz ? "bg-emerald-500/10 border-emerald-500/20" : "bg-blue-500/10 border-blue-500/20"}`}
            >
              <p
                className={`text-sm ${isLiveQuiz ? "text-emerald-400" : "text-blue-400"}`}
              >
                {isLiveQuiz
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {CLASS_OPTIONS.map((cls) => (
                      <option key={cls} value={cls} className="bg-[#13151f]">
                        {cls}
                      </option>
                    ))}
                  </select>
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {PAPER_OPTIONS.map((p) => (
                      <option key={p} value={p} className="bg-[#13151f]">
                        {p}
                      </option>
                    ))}
                  </select>
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {CHAPTER_OPTIONS.map((ch) => (
                      <option key={ch} value={ch} className="bg-[#13151f]">
                        Chapter {ch}
                      </option>
                    ))}
                  </select>
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {QUESTION_TYPE_OPTIONS.map((qt) => (
                      <option key={qt} value={qt} className="bg-[#13151f]">
                        {qt.toUpperCase()}
                      </option>
                    ))}
                  </select>
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {DIFFICULTY_OPTIONS.map((diff) => (
                      <option key={diff} value={diff} className="bg-[#13151f]">
            ...
            <div className="max-h-72 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {loading ? (
                ...
                filteredQuestions.map((q) => {
                  const selected = formData.question_ids.includes(
                    q.question_id,
                  );
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border transition-all ${selected ? "bg-purple-500/10 border-purple-500/30" : "bg-white/5 border-white/5 hover:bg-white/10"}`}
                    >
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center flex-shrink-0 ${selected ? "bg-purple-600 border-purple-500" : "border-gray-600"}`}
                      >
                        {selected && (
                          <svg
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded flex-shrink-0 ${q.difficulty === "easy" ? "bg-emerald-500/10 text-emerald-400" : q.difficulty === "medium" ? "bg-amber-500/10 text-amber-400" : "bg-rose-500/10 text-rose-400"}`}
                      >
                        {q.difficulty}
                      </span>
                    </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl font-bold hover:shadow-lg transition-all disabled:opacity-50"
          >
            {loading ? (
