...
const LiveQuiz = () => {
  ...
  useEffect(() => {
    getAllExams()
      .then((r) =>
        setExams(r.data.data.filter((e) => e.exam_type === "live_quiz")),
      )
      ...
      <main className="max-w-7xl mx-auto px-6 pt-6 pb-20">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">
            Live <span className="text-emerald-400">Quiz</span>
          </h1>
          <p className="text-gray-400">
            Manage and host real-time quiz sessions
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl p-6 mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Create an exam with type "Live Quiz", start it, and share the
              access code with students. They join using /join-quiz.
            </p>
          </div>
        </div>
            ...
            {exams.map((exam) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div
                    className={`w-3 h-3 rounded-full ${exam.status === "ongoing" ? "bg-emerald-500 animate-pulse" : exam.status === "completed" ? "bg-gray-500" : "bg-amber-500"}`}
                  ></div>
