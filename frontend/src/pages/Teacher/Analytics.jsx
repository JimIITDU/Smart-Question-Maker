...
const Analytics = () => {
  ...
  const difficultyData = [
    {
      name: "Easy",
      value: data.questions.filter((q) => q.difficulty === "easy").length,
    },
    {
      name: "Medium",
      value: data.questions.filter((q) => q.difficulty === "medium").length,
    },
    {
      name: "Hard",
      value: data.questions.filter((q) => q.difficulty === "hard").length,
    },
  ];
  ...
  const typeData = [
    {
      name: "MCQ",
      value: data.questions.filter((q) => q.question_type === "mcq").length,
    },
    {
      name: "True/False",
      value: data.questions.filter((q) => q.question_type === "true_false")
        .length,
    },
    {
      name: "Descriptive",
      value: data.questions.filter((q) => q.question_type === "descriptive")
        .length,
    },
  ];
  ...
  const completedExams = data.exams.filter(
    ...
    <>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">
          Question <span className="text-cyan-400">Analytics</span>
        </h1>
        <p className="text-gray-400">
          Analyze your question distribution and performance
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {Array.from({ length: 50 }, (_, i) => (i + 1).toString()).map(
                  (ch) => (
                ...
                <BarChart data={typeData}>
                  ...
                  <Bar dataKey="value">
                    {typeData.map((_, index) => (
                      <Cell
                        key={index}
                        fill={["#8b5cf6", "#06b6d4", "#ec4899"][index]}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            {data.exams.length === 0 ? (
              ...
              <div className="space-y-3">
                {data.exams.slice(0, 5).map((exam) => (
                  <div
                    key={exam.exam_id}
                    className="flex justify-between items-center p-5 bg-white/5 rounded-xl border border-white/5"
                  >
                    <div>
                    ...
                    <span
                      className={`px-4 py-1 text-xs font-bold rounded-full capitalize border ${
                        exam.status === "ongoing"
