...
const ManageExams = () => {
  ...
  useEffect(() => {
    getAllExams().then(r => setExams(r.data.data)).catch(() => toast.error('Failed to load exams')).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Manage <span className="text-purple-400">Exams</span></h1>
          <p className="text-gray-400">View, start, and monitor all your exams</p>
        </div>
            ...
            {exams.map((exam) => (
              <div key={exam.exam_id} className="bg-white/5 border border-white/5 hover:border-white/10 rounded-2xl p-6 transition-all">
                <div className="flex justify-between items-start mb-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${statusColor(exam.status)}`}>{exam.status}</span>
                  <span className="text-xs text-gray-500 bg-white/5 px-2 py-1 rounded">{exam.exam_type}</span>
                </div>
                ...
                <div className="space-y-1 mb-4 text-sm text-gray-400">
                  ...
                  {exam.access_code && <p className="font-mono text-purple-400">Code: {exam.access_code}</p>}
                </div>
