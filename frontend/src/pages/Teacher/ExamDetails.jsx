...
const ExamDetails = () => {
  ...
  const pendingEvaluations = results.filter(
    (r) => r.question_type === 'descriptive' && (r.evaluated_by === 'pending' || !r.evaluated_by)
  ).length
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-white">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-28 pb-20 space-y-6">
                ...
                <div key={q.question_id} className="flex items-start gap-4 p-4 bg-white/5 rounded-xl border border-white/5">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {q.correct_option && (
                        <span className="text-xs text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded">Correct: {q.correct_option}</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
          <h2 className="text-xl font-bold text-white mb-4">Student Results ({Object.keys(groupedResults).length})</h2>
          {Object.keys(groupedResults).length === 0 ? (
            <p className="text-gray-500">No submissions yet</p>
          ) : (
            <div className="space-y-6">
              {Object.entries(groupedResults).map(([studentId, studentData]) => {
                const percentage = studentData.totalMarks > 0 
                  ? ((studentData.obtainedMarks / studentData.totalMarks) * 100).toFixed(1) 
                  : 0
                const passed = percentage >= 50

                return (
                  <div key={studentId} className="bg-white/5 rounded-xl border border-white/5 overflow-hidden">
                    <div className="p-4 flex justify-between items-center bg-white/5">
                      <div>
                        <p className="text-white font-semibold">{studentData.student_name}</p>
                        <p className="text-gray-500 text-sm">{studentData.student_email}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-bold text-white">{studentData.obtainedMarks} <span className="text-gray-500 text-lg">/ {studentData.totalMarks}</span></p>
                        <p className={`text-sm font-bold ${passed ? 'text-emerald-400' : 'text-rose-400'}`}>{percentage}% - {passed ? 'Pass' : 'Fail'}</p>
                      </div>
                    </div>
                        ...
                        <div key={idx} className="p-4">
                            ...
                            <div className="flex gap-2">
                              ...
                              {answer.evaluated_by && (
                                <span className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${
                                  ...
                                  {answer.evaluated_by === 'llm' ? <FiCpu className="text-[10px]" /> : <FiCheckCircle className="text-[10px]" />}
                                  {answer.evaluated_by}
                                </span>
                              )}
                            </div>
                            <span className="text-white font-bold">{answer.marks_obtained} / {answer.max_marks}</span>
                          </div>
                          
                          <p className="text-gray-300 text-sm mb-2">{answer.question_text}</p>
                          
                          {answer.question_type === 'descriptive' && (
                            <div className="space-y-2">
                              <div className="bg-white/5 rounded-lg p-3">
                                <p className="text-xs text-gray-500 uppercase mb-1">Student Answer</p>
                                <p className="text-gray-300 text-sm">{answer.descriptive_answer || 'No answer provided'}</p>
                              </div>
                              {answer.feedback && (
                                <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-lg p-3">
                                  <p className="text-xs text-indigo-400 uppercase mb-1 flex items-center gap-1">
                                    <FiCpu className="text-[10px]" /> LLM Feedback
                                    {answer.confidence_score && (
                                      <span className="text-gray-500">(Confidence: {(answer.confidence_score * 100).toFixed(0)}%)</span>
                                    )}
                                  </p>
                          ...
                          {answer.question_type === 'mcq' && (
                            <div className="flex gap-4 text-sm">
                              <span className="text-gray-500">Correct: <span className="text-emerald-400">{answer.correct_option}</span></span>
                            </div>
                          )}
                        </div>
                      ))}
