...
import { FiArrowLeft, FiArrowRight, FiCpu, FiFilter, FiZap, FiSave, FiCheckCircle, FiEdit2, FiXCircle, FiRefreshCw, FiUpload, FiPenTool, FiBookOpen, FiFileText } from 'react-icons/fi'
...
const AIQuestionGenerator = () => {
  ...
  const regenerateRejected = async () => {
    const rejectedCount = questions.filter((q) => q._status === 'rejected').length;
  ...
  const acceptedCount = questions.filter((q) => q._status === 'accepted').length;
  const rejectedCount = questions.filter((q) => q._status === 'rejected').length;
      ...
      <main className="max-w-5xl mx-auto px-6 pt-28 pb-20 space-y-8">
        ...
        <section className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
                Paper <span className="text-gray-500 normal-case font-normal">(optional)</span>
              </label>
              <select
                name="paper"
                value={filters.paper}
            ...
            <div className="md:col-span-2">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
                Chapter Name <span className="text-gray-500 normal-case font-normal">(optional)</span>
              </label>
              <input
                type="text"
                name="chapter_name"
            ...
            <div className="md:col-span-2">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
                Topic <span className="text-gray-500 normal-case font-normal">(optional)</span>
              </label>
              <input
                type="text"
                name="topic"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-gray-400">
                  Write questions yourself. Opens the Create Question form with your filters pre-filled.
                </p>
              </button>
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h4 className="font-bold text-white">Based on previous questions</h4>
                </div>
                <p className="text-sm text-gray-400">
                  AI studies your existing question bank for this subject/chapter and generates similar questions. Optionally add a PDF or paste text to guide it further.
                </p>
              </button>
              ...
              >
                ...
                <p className="text-sm text-gray-400">
                  Upload a PDF or paste text. AI generates questions strictly from that content. At least one input is required.
                </p>
              </button>
              ...
              >
                ...
                <p className="text-sm text-gray-400">
                  AI generates questions using only the subject, chapter and topic you selected above. No extra input needed.
                </p>
              </button>
            </div>
          </section>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-6">
              ...
              {(mode === 'previous' || mode === 'pdf_text') && (
                <>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
                      {mode === 'pdf_text' ? 'Upload PDF (required if no text)' : 'Upload PDF (optional)'}
                    </label>
                    <div
...
value={editForm.expected_answer}
                              onChange={(e) =>
                                setEditForm((f) => ({ ...f, expected_answer: e.target.value }))
                              }
                              rows={3}
                              className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 resize-none"
                            />
                        ...
                        {q.question_type === 'true_false' && (
                          <div>
                            ...
                            <select value={editForm.correct_option} onChange={(e) => setEditForm((f) => ({ ...f, correct_option: e.target.value }))} className="w-full bg-[#0B1120] border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500">
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => saveEdit(q._id)} className="flex-1 py-2 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 flex items-center justify-center gap-2"><FiSave /> Save</button>
                          <button onClick={cancelEdit} className="px-4 py-2 bg-white/10 text-gray-400 border border-white/10 rounded-xl">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <>
                            ...
                            {(() => {
                              ...
                              return ['a','b','c','d'].map(opt => q[`option_text_${opt}`] && (
                                <div key={opt} className={`text-xs p-2 rounded-lg border ${correctOpts.includes(opt.toUpperCase()) ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-white/5 border-white/5 text-gray-400'}`}>
                                  <span className="font-bold uppercase mr-1">{opt}.</span>{q[`option_text_${opt}`]}{correctOpts.includes(opt.toUpperCase()) && <span className="ml-1 text-emerald-300">✓</span>}
                                </div>
                              ));
                            })()}
                          </div>
                        ...
                        {q.question_type === 'true_false' && (
                          <div className="flex gap-2 mb-3">
                            {['True','False'].map(opt => (
                              <div key={opt} className={`text-xs px-3 py-2 rounded-lg border ${q.correct_option === opt ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-white/5 border-white/5 text-gray-400'}`}>
                                {opt}{q.correct_option === opt && <span className="ml-1 text-emerald-300">✓</span>}
                              </div>
                            ))}
                          </div>
                        )}
                        {q.question_type === 'descriptive' && q.expected_answer && (
                          <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-3">
                            <p className="text-xs text-gray-500 uppercase mb-1">Model Answer</p>
                            <p className="text-sm text-gray-300">{q.expected_answer}</p>
                          </div>
                        )}
                        <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-white/5">
                          <div className="flex items-center gap-1">
                            ...
                            {['easy','medium','hard','auto'].map(d => (
                              <button key={d} onClick={() => setDifficultyOverride(q._id, d)} className={`text-xs px-2 py-1 rounded-lg border ${(d === 'auto' && !q._difficultyOverride) || q._difficultyOverride === d ? d === 'easy' ? 'bg-emerald-500/20 text-emerald-300' : d === 'medium' ? 'bg-yellow-500/20 text-yellow-300' : d === 'hard' ? 'bg-red-500/20 text-red-300' : 'bg-purple-500/20 text-purple-300' : 'bg-white/5 text-gray-500'}`}>
                                {d === 'auto' ? 'Auto' : d.charAt(0).toUpperCase() + d.slice(1)}
                              </button>
                            ))}
                          </div>
                          <div className="flex-1" />
                          <button onClick={() => acceptQuestion(q._id)} disabled={q._status === 'accepted'} className="px-3 py-2 bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 rounded-xl disabled:opacity-50 flex items-center gap-1 text-sm"><FiCheckCircle /> Accept</button>
                          <button onClick={() => startEdit(q)} className="px-3 py-2 bg-blue-600/20 text-blue-400 border border-blue-500/30 rounded-xl flex items-center gap-1 text-sm"><FiEdit2 /> Edit</button>
                          <button onClick={() => rejectQuestion(q._id)} disabled={q._status === 'rejected'} className="px-3 py-2 bg-red-600/20 text-red-400 border border-red-500/30 rounded-xl disabled:opacity-50 flex items-center gap-1 text-sm"><FiXCircle /> Reject</button>
                        </div>
                      </>
                    )}
                  </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
              <p className="text-sm text-gray-400">You have accepted <span className="text-emerald-400 font-bold">{acceptedCount}</span> questions. You rejected <span className="text-red-400 font-bold">{rejectedCount}</span> questions.</p>
              {rejectedCount > 0 && (
                <button onClick={regenerateRejected} disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-orange-600 to-amber-600 text-white rounded-xl font-bold disabled:opacity-50">
                  {loading ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Regenerating...</> : <><FiRefreshCw /> Regenerate {rejectedCount} rejected</>}
                </button>
              )}
              <button onClick={saveAccepted} disabled={loading || acceptedCount === 0} className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-bold disabled:opacity-50">
                {loading ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> Saving...</> : <><FiSave /> Save {acceptedCount} accepted to bank</>}
              </button>
            </div>
          </section>
        )}
