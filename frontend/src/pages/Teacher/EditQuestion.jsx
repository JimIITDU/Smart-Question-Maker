  ...
  useEffect(() => {
    getQuestionById(id)
      .then((res) => {
        ...
        if (q.correct_option) {
          const options = q.correct_option
            .split(",")
      ...
      <main className="max-w-4xl mx-auto px-6 pt-28 pb-20">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold hover:shadow-lg transition-all disabled:opacity-50"
          >
            {loading ? (
              <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <>
                <FiSave /> Update Question
              </>
            )}
          </button>
        </form>
